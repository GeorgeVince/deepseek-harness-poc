...
The frontend restores messages and tool calls from PostgreSQL. Provider-exposed reasoning summaries appear as one live Process block per question-and-response turn, showing only the latest step while work is underway and collapsing into an optional full history when the response finishes; ordinary tool calls stay hidden, while generated workbooks appear as download cards and failed attempts remain quietly collapsed. These short summaries are not the model's private chain-of-thought. After a backend restart, the first turn replays the message transcript into a fresh Harness runtime session; Harness's full logs remain in ignored `.dsh/sessions/`. Restart the backend after changing credentials.
